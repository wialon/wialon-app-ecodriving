    ...
    function Plot(placeholder, data_, options_, plugins) {
        ...
        function processData() {
            ...
            for (i = 0; i < series.length; ++i) {
                ...
                if (s.bars.show) {
                    ...
                    var delta;
                    var barWidth = format.length === 3 ? points[points.length-1] : s.bars.barWidth;
                    switch (s.bars.align) {
                        case "left":
                            delta = 0;
                            break;
            ...
            $.each(allAxes(), function (_, axis) {
                ...
                for (var i = 0; i < axis.ticks.length; ++i) {
                    ...
                    if (axis.direction == "x") {
                        ...
                        if (axis.position == "left") {
                            x = box.left + box.width - box.padding + 8;
                            halign = "right";
                        } else {
        ...
        function drawSeriesBars(series) {
            function plotBars(datapoints, barLeft, barRight, offset, fillStyleCallback, axisx, axisy) {
				var tmp = (datapoints.pointsize === 3);
                var points = datapoints.points, ps = datapoints.pointsize;

                for (var i = 0; i < points.length; i += ps) {
                    if (points[i] == null)
                        continue;
                    if (series.bars.strokeColor)
                        ctx.strokeStyle = series.bars.strokeColor;
					drawBar(points[i], points[i + 1], tmp ? 0 : points[i + 2], barLeft,
							barRight + (tmp ? points[i + 2] : 0), offset, fillStyleCallback, 
							axisx, axisy, ctx, series.bars.horizontal, series.bars.lineWidth);
                }
            }
            ...
            for (i = series.length - 1; i >= 0; --i) {
                ...
                if (s.bars.show && !item) { // no other point can be nearby
                    ...
                    for (j = 0; j < points.length; j += ps) {
                        ...
                        if (s.datapoints.format.length === 3) {
                            barWidth = points[j + 2];
                            switch (s.bars.align) {
                                case "left":
                                    barLeft = 0; break;
                                case "right":
                                    barLeft = -barWidth; break;
                                default:
                                    barLeft = -barWidth / 2;
                            }
                        ...
                        if (series[i].bars.horizontal ?
                            ...
                            (mx >= x + barLeft && mx <= x + barRight && my <= y && my >= 0
        ...
        function drawBarHighlight(series, point) {
            var highlightColor = (typeof series.highlightColor === "string") ? series.highlightColor : $.color.parse(series.color).scale('a', 0.5).toString(),
                ...
                barWidth = point.length === 3 ? point[2] : series.bars.barWidth;
                barLeft = series.bars.align == "left" ? 0 : -barWidth/2;

            octx.lineWidth = series.bars.lineWidth;
            octx.strokeStyle = highlightColor;
            var barRight = barLeft + barWidth;
            drawBar(point[0], point[1], 0, barLeft, barRight,
